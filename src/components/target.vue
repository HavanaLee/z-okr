        ...
        <div style="margin-left: 24px;">
          <div class="obj_input_cont">
            ...
            <el-input type="textarea" v-model="props.list.content" class="obj_input_content"
              placeholder="填写目标，按 Enter 键保存" @blur="blurInput($event, 'target')" maxlength="500" resize="none"
              :autosize="true" @focus="foucusInput($event, 'target')"></el-input>
          </div>
        </div>
      </div>
    </div>
    <!-- 指标部分 -->
    <div class="KeyResult" v-for="(v, i) in deep_list.obj.indicator" :key="v.id">
      <div class="key_res_info">
        <el-input v-model="v.content" type="textarea" placeholder="填写指标，用他来实现目标" maxlength="500" resize="none"
          :autosize="true" @focus="foucusInput($event, 'index', i)" @blur="blurInput($event, 'index', i)"></el-input>
        <div class="key_res_act">
          ...
          <div class="key_res_weight" v-show="v.isFocused">
            <span>
              <svg width="1em" height="1em" fill="currentColor" aria-hidden="true" focusable="false" class="">
...
const foucusInput = function (e: FocusEvent, source: string, i?: number) {
  ...
  while ((!target.classList.contains('obj_row') && source === 'target') || (!target.classList.contains('KeyResult') && source === 'index')) {
...
function addIndicator() {
  deep_list.obj.indicator.push({ content: '', score: 1, weigtht: '100%', id: generateUUID(), isFocused: true })
}
</script>
